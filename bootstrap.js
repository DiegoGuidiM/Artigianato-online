...
// Esegue tutto dentro una singola transazione sullo stesso client.
...
async function bootstrap() {
  ...
  try {
    ...
    // FK su location.id_host se manca (Postgres non ha IF NOT EXISTS per i constraint)
    await client.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          ...
          WHERE table_name = 'location'
            ...
            AND constraint_name = 'location_id_host_fkey'
        ) THEN
          ALTER TABLE location
            ADD CONSTRAINT location_id_host_fkey
            FOREIGN KEY (id_host) REFERENCES users(id_user) ON DELETE SET NULL;
        END IF;
      END $$;
    `);
...
// CLI: esegui `node src/bootstrap.js`
